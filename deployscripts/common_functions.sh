...
# Set the dir where to deploy the revision
#
# The appspec.yml line with the dir path to deploy the revision into
# is of the format:  '   destination: /path/to/deploy/dir'
#
# IFS='' -- prevents leading/trailing whitespace from being trimmed.
# -r -- prevents backslash escapes from being interpreted.
#
# *MUST* use '$(dirname $0)/../' as just '../' doesn't work for Codedeploy agent
#
regex_destination=' *destination: ([a-zA-Z0-9/-]+)'

while IFS='' read -r line; do
  if [[ "$line" =~ $regex_destination ]]; then
    TARGET_DIR=${BASH_REMATCH[1]}
    break
  fi
done < $(dirname $0)/../appspec.yml

# Log file that holds the version of the serivce being deployed/started
DEPLOY_LOG=deploy.log

# redirect stderr and stdout to syslog logger with tag
exec 1> >(logger -t AWSCodeDeployHooks) 2>&1
